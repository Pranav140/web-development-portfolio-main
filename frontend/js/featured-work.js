...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // --- Fetch all projects and featured projects from backend ---
  let allProjects = [];
  let featuredProjects = [];
  try {
    [allProjects, featuredProjects] = await Promise.all([
      apiFetch("/api/projects"),
      apiFetch("/api/projects/featured"),
    ]);
  } catch (err) {
    console.error("Failed to load projects:", err);
    // Fallback: generate dummy data to keep animations intact
    allProjects = Array.from({ length: 10 }, (_, i) => ({
      title: `Project ${i + 1}`,
      image: `/images/work-items/work-item-${i + 1}.jpg`,
    }));
    featuredProjects = allProjects.slice(0, 5);
  }

  // --- Populate .featured-titles with API data ---
  const featuredTitlesContainer = document.querySelector(".featured-titles");
  if (featuredTitlesContainer) {
    // First wrapper: static header
    let titlesHTML = `
      <div class="featured-title-wrapper">
        <h1 class="featured-title">Featured Projects</h1>
      </div>`;
    // Subsequent wrappers: one per featured project
  ...
  // Update the project count text in the featured-work footer
  const projectCountEl = document.querySelector(".featured-work-footer p.mn:first-child");
  if (projectCountEl) {
    projectCountEl.textContent = `Project Portfolio [ ${allProjects.length} ]`;
  }
  ...
  const initAnimations = () => {
    ...
    // Create section indicators (1 header + N featured projects) and progress dots
    const totalSections = featuredProjects.length + 1; // +1 for the "Featured Projects" header
    ...
    // Create image cards dynamically from API projects
    ...
    allProjects.forEach((project, idx) => {
      ...
      img.alt = project.title || `featured work image ${i}`;
      featuredImgCard.appendChild(img);
      // Set initial position (cycle through predefined positions if more projects than positions)
      const position = featuredCardPos[idx % featuredCardPos.length];
