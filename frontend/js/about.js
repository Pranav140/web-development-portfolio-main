...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // --- Populate owner data from the backend API ---
  try {
    const owner = await apiFetch("/api/owner");

    // About hero: first name heading
    const aboutNameEl = document.querySelector(".about-hero-header h1:nth-child(2)");
    if (aboutNameEl) aboutNameEl.textContent = owner.firstName;

    // About bio
    const aboutBioEl = document.querySelector(".about-hero-bio p.ss");
    if (aboutBioEl) aboutBioEl.textContent = owner.bio;

    // About tagline
    const aboutTaglineEl = document.querySelector(".about-hero-bio p.mn");
    if (aboutTaglineEl) aboutTaglineEl.textContent = owner.tagline;

    // Portrait
    const portraitEl = document.querySelector(".about-hero-portrait img");
    if (portraitEl) {
      portraitEl.src = owner.portrait;
      portraitEl.alt = `${owner.name} Portrait`;
    }

    // Services-header portrait
    const servicesPortraitEl = document.querySelector(".services-profile-icon img");
    if (servicesPortraitEl) servicesPortraitEl.src = owner.portrait;

    // Nav social links
    const githubLinks = document.querySelectorAll("a[href*='github.com']");
    githubLinks.forEach((el) => { el.href = owner.social.github; });
    const linkedinLinks = document.querySelectorAll("a[href*='linkedin.com']");
    linkedinLinks.forEach((el) => { el.href = owner.social.linkedin; });
    const twitterLinks = document.querySelectorAll("a[href*='x.com']");
    twitterLinks.forEach((el) => { el.href = owner.social.twitter; });
    const emailLinks = document.querySelectorAll(`a[href^="mailto:"]`);
    emailLinks.forEach((el) => {
      el.href = `mailto:${owner.email}`;
      if (el.textContent.includes("@")) el.textContent = owner.email;
    });

    // Footer header name
    const footerHeaderEl = document.querySelector(".footer-header h1");
    if (footerHeaderEl) footerHeaderEl.textContent = owner.name;

    // Resume link
    const resumeLink = document.querySelector(".resume-link");
    if (resumeLink && owner.resumeLink) resumeLink.href = owner.resumeLink;

    // Hero header names
    const heroHeader1 = document.querySelector(".hero-header-1 h1");
    const heroHeader2 = document.querySelector(".hero-header-2 h1");
    if (heroHeader1) heroHeader1.textContent = owner.firstName;
    if (heroHeader2) heroHeader2.textContent = owner.lastName;
  } catch (err) {
    console.error("Failed to load owner data:", err);
    // Silently fall back to whatever is in the HTML
  }
