...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Fetch project images from backend for particles
    let imagePaths = [];
    try {
      const projects = await apiFetch("/api/projects");
      imagePaths = projects.map((p) => p.image);
    } catch (err) {
      console.error("Failed to load project images for footer:", err);
      // Fallback to static paths
      imagePaths = Array.from({ length: 10 }, (_, i) => `/images/work-items/work-item-${i + 1}.jpg`);
    }
