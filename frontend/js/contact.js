...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Fetch image paths for trail from backend
  let images = [];
  try {
    const projects = await apiFetch("/api/projects");
    images = projects.slice(0, config.imageCount).map((p) => p.image);
  } catch (err) {
    console.error("Failed to load project images for trail:", err);
    // Fallback to static paths if API is unavailable
    images = Array.from({ length: config.imageCount }, (_, i) => `/images/work-items/work-item-${i + 1}.jpg`);
  }
  ...
  // Handle form submission – POST data to the backend API
