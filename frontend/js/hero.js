...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    projectImages = projects.map((p) => p.image);
  } catch (err) {
    console.error("Failed to load project images for hero:", err);
    // Fallback to static paths
    projectImages = Array.from({ length: 10 }, (_, i) => `/images/work-items/work-item-${i + 1}.jpg`);
  }
