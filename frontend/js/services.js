...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // --- Fetch services from backend and render cards ---
  ...
  try {
    ...
    servicesSection.innerHTML = services
      .map(
        (s, i) => `
        <div class="service-card" id="service-card-${i + 1}">
    ...
    // If API fails, existing hardcoded cards in HTML (if any) will remain
  }
